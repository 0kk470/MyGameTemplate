...
namespace UnityEngine.UI
{
    ...
    public abstract class LoopScrollRect : UIBehaviour, IInitializePotentialDragHandler, IBeginDragHandler, IEndDragHandler, IDragHandler, IScrollHandler, ICanvasElement, ILayoutElement, ILayoutGroup
    {
        ...
        //==========LoopScrollRect==========
        public void BindCellComponent<T>() where T:Component
        {
            var t = typeof(T);
            dataSource?.SetComponentType(t);
            prefabSource?.SetComponentType(t);
        }

        //Listener
        public void AddListenerOnData(Action<Component, int> onData)
        {
            if (dataSource != null)
                dataSource.onData += onData;
        }

        public void AddListenerOnReturn(Action<Component> onReturn)
        {
