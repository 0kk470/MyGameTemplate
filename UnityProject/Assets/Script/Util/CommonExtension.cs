...
namespace Saltyfish.Util
{
    public static class CommonExtension
    {
        ...
        public static T PickRandomElement<T>(this ICollection<T> elementSet, bool removePicked = true, System.Random rand = null)
        {
            ...
            var randomIdx = -1;
            if (rand == null)
                randomIdx = GameRand.InclusiveRange(0, elementSet.Count - 1);
            else
        ...
        /// </summary>
        /// <returns></returns>
        public static IEnumerable<T> TakeLast<T>(this IEnumerable<T> enumrables, int cnt)
        {
            return enumrables.Skip(Math.Max(0, enumrables.Count() - cnt));
        }
        ...
        public static T SafeGet<T>(this IList<T> source, int idx, bool logError = false)
        {
            ...
            {
                if (logError)
                    Debug.LogError("Error, idx is out if range");
                return default(T);
            }
            return source[idx];
        }
